...
let tag = '', post = ''
let tagPref = 'explore/tags/', postPref = 'p/'

process.argv.slice(2).map(arg => {
  if (arg.includes('--tag'))
    tag = arg.split('=')[1]
  else if (arg.includes('--post'))
    post = arg.split('=')[1]
})
  ...
  axios.get(url)
    .then(response => {
      ...
      response.data.graphql.hashtag.edge_hashtag_to_media.edges.map(post => {
        ...
        const file = fs.createWriteStream(`./files/${tag}/${post.node.shortcode}.jpg`);
...
function getPost() {
  ...
  axios.get(url)
    .then(response => {
      ...
      const request = https.get(response.data.graphql.shortcode_media.display_url, function (response) {
        response.pipe(file);
      });
    })
    .catch(err => {
